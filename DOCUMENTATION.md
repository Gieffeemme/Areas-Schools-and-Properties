...
- **Scotland** — **deprivation (SIMD 2020v2) + the schools register now shipped** (see §3/5/6/9). Scottish
  ...
  grade and there's no per-school results feed (same shape as Wales/NI). Note police.uk has no Police
  Scotland data, so the **crime** layer is the one remaining Scotland gap (England/Wales/NI only).
