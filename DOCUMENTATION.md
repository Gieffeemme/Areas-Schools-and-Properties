...
src/
  ...
  lib/   (one concern each)
    ...
    crime.ts        fetchCrime()  ·  prices.ts  fetchPrices()/fetchAddressSales()  ·  flood.ts  fetchFlood()  ·  transport.ts  nearestStations() (nearest rail/metro/tram from committed stations.json, runtime fs read)
...
- **Per-property report: what's free vs not.** A true type-the-address autocomplete needs PAF/AddressBase,
  ...
  count (stations within 1 mile — still a live Overpass call). Door-to-door commute *times* would need a
  paid routing/journey-planner API, deliberately out of scope. `/api/area` still skips caching when
  `transport` is null, but with committed data that only happens if the dataset file is absent (a deploy
  bug), not from flakiness.
