...
- **Wales** — deprivation now **shipped** (WIMD 2025, see §5/6/9). Schools still excluded: *present in
  GIAS but* carry no statutory age fields to derive phase from, and aren't covered by our England
  Ofsted/DfE enrichment. Remaining Welsh schools pipeline: a Welsh phase derivation (or the Welsh-Gov
  schools address list) + **Estyn** inspections (no Ofsted-style single grade — narrative only) +
  **Welsh-Government** results (sparse; no league tables).
