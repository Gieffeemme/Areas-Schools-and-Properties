...
src/
  app/
    ...
    api/
      area/route.ts            geocode + schools + crime + prices + amenities + ev-charging + transport + broadband + mobile + noise + air-quality + census → AreaReport (cached 6h)
  ...
  components/
    ...
    DeprivationPanel · DemographicsPanel · CrimePanel · PricePanel · AmenitiesPanel · TransportPanel · EvChargingPanel · BroadbandPanel · MobilePanel · NoisePanel · AirQualityPanel · RankingsPanel  (area panels)
...
annual mean + WHO-guideline band, an area panel pairing with noise, via `etl:air-quality`), **mobile coverage** (Ofcom Connected Nations
...
Every originally-queued free dataset has shipped; **EV charging** was then added as the first from a new
"what other England free data exists" sweep. Further free, geography-joinable candidates not yet built:
**bus stops** (NaPTAN), **DfT journey-time/connectivity** stats (mins to GP/school/shops by LSOA; ~2019
vintage), **expanded Census 2021** (car/van availability, ethnicity, health — same Nomis pipeline),
**OHID Fingertips** local-health indicators, **EA bathing-water** quality (coastal), and **ONS small-area
income**. (**Census 2021 demographics**, **CQC health/care ratings**,
**air quality**, **mobile coverage**, **planning constraints** and **EV charging** shipped this round.
