...
src/
  app/
    ...
    api/
      area/route.ts            geocode + schools + crime + prices + amenities + transport + broadband + noise + air-quality + census → AreaReport (cached 6h)
  ...
  components/
    ...
    DeprivationPanel · DemographicsPanel · CrimePanel · PricePanel · AmenitiesPanel · TransportPanel · BroadbandPanel · NoisePanel · AirQualityPanel · RankingsPanel  (area panels)
...
annual mean + WHO-guideline band, an area panel pairing with noise, via `etl:air-quality`),
...
distinct from the shipped planning *applications* via PlanIt); and **mobile coverage** (Ofcom Connected
Nations, reusing the broadband ETL). (**Census 2021 demographics**, **CQC health/care ratings** and
**air quality** shipped this round. CQC was *not* built on the key-gated CQC Syndication API — that API has no radius search and
