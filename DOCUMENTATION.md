...
|-------|:---:|:---:|:---:|:---:|
| School pins + phase | ✓ GIAS | ✓ Welsh register | ✓ SG register | ✓ DE register |
| School Ofsted grade + DfE results | ✓ | ✗ (Estyn → My Local School) | ✗ (Education Scotland → Parentzone) | ✗ (ETI → DE Schools Plus) |
| Nurseries (Ofsted Early Years) | ✓ | ✗ | ✗ | ✗ |
| Deprivation (per-domain) | ✓ IMD 2019 | ✓ WIMD 2025 | ✓ SIMD 2020v2 | ✓ NIMDM 2017 |
| Crime | ✓ street-level (police.uk) | ✓ street-level | ✓ council-area only (no Police Scotland on police.uk → SG recorded crime) | ✓ street-level |
...
| Green space & gardens (ONS) | ✓ | ✓ | ✓ (garden % gaps) | ✗ (GB-only dataset) |
...
| NI schools | `ni-schools.json` | **Dept of Education NI** "school level" data | postcode centroid | ~1,100 NI schools — `nation:"Northern Ireland"`, type→phase, enrolment, mgmt type, Irish-medium, grammar(selective); no Ofsted/results (ETI → DE Schools Plus) |
| Scottish schools | `scotland-schools.json` | **Scottish Government** "School Roll and Locations" | precise lat/lng (in the file) | ~2,480 Scotland schools — `nation:"Scotland"`, type→phase, roll, denomination; no Ofsted/results (Education Scotland → Parentzone) |
...
Scotland → SIMD-2020v2 by `codes.lsoa11` (`simd-by-datazone.json`), Northern Ireland → NIMDM-2017 by
`codes.lsoa11` (`nimdm-by-soa.json`). Each dataset is keyed to a single nation's codes, so the lookups
self-gate — only one populates per postcode. **All four UK nations now have deprivation.**
...
| `ni-schools.json` | (array) | `etl:ni-schools` | NI school pins: name, postcode, lat/lng, phase (from type), `kind`, enrolment, management type, **Irish-medium**, grammar (selective), DE ref (Northern Ireland) |
| `scotland-schools.json` | (array) | `etl:scotland-schools` | Scottish school pins: name, postcode, lat/lng, phase (from type), `kind`, roll, denomination, seed code (Scotland) |
...
| `nimdm-by-soa.json` | SOA code (`95…`) | `etl:nimdm` | NIMDM-2017 overall rank/decile + decile for each of the 7 NI domains (Northern Ireland) |
| `scotland-crime-by-laua.json` | council-area code (`S12…`) | `etl:scotland-crime` | council-area recorded crime — rate per 10k + 5 crime groups + Scotland average (Scotland; police.uk has no Police Scotland) |
| `scotland-council-tax-by-laua.json` | council-area code (`S12…`) | `etl:scotland-council-tax` | council-area council tax — band mix (counts A–H) + £/band (Scotland; no neighbourhood band mix is published free) |
...
| `greenspace.json` | `byLsoa` (LSOA-2011/data-zone) + `byMsoa` (MSOA) | `etl:greenspace` | ONS green-space access: nearest park distance + count within 1 km by LSOA-2011 (`codes.lsoa11`), private-garden share % by MSOA (`codes.msoa`); Great Britain |
...
| `etl:ni-schools` | `ni-schools.json` | **Dept of Education NI** "school enrolment - school level data" — four date-stamped **XLSX** (primary/post-primary/special/nursery) from `education-ni.gov.uk/publications/school-enrolment-school-level-data-202425`, via SheetJS. Each has a "reference data" sheet (name, postcode, school type, management, Irish-medium) + an "Enrolments" sheet (total). Type→phase (PREP→Primary, GRAMMAR→Secondary+selective); postcode-geocoded. No dedupe needed (NI BT-postcodes never collide with England GIAS). No Ofsted/results — link to the DE **Schools Plus** directory (no clean per-school deep-link exists). **Per-file URLs are date-stamped → update from the publication page on refresh.** |
| `etl:scotland-schools` | `scotland-schools.json` | **Scottish Government** "School Roll and Locations" — one **XLSX inside a zip** (`maps.gov.scot/ATOM/shapefiles/SG_SchoolRoll_2023_Table.zip`; needs `unzip` on PATH, read via SheetJS). The richest register: name, postcode, **lat/lng (already geocoded)**, SchoolType→phase, PupilRoll, Denomination→religion, seed code — for every publicly-funded primary/secondary/special school. No Ofsted/results — link to **Parentzone** Scotland. Vintage 2023 (fine for a register). |
...
| `etl:nimdm` | `nimdm-by-soa.json` | **NISRA** NI Multiple Deprivation Measure **2017** "SOA" **CSV** from **Open Data NI** (`admin.opendatani.gov.uk/dataset/…/nimdm2017-soa.csv`). Keyed by **SOA** code (890 SOAs, `95…`) → joins postcodes.io `codes.lsoa11` (NOT `codes.lsoa`, the 2021 NI data zone `N21…`). 7 domains. 890/10 = 89 exactly, so deciles are uniform: `decile = ceil(rank/89)`. CSV, so no SheetJS. |
| `etl:scotland-crime` | `scotland-crime-by-laua.json` | **Scottish Government** "Recorded Crimes and Offences" cube, full **CSV** download from `statistics.gov.scot/downloads/cube-table?uri=…/recorded-crime` (~12 MB, OGL). Filters to the latest year + `FeatureType=Council Area`; keeps "All Crimes" + the 5 crime groups (count + Ratio=rate per 10k), plus the `Country` (Scotland, `S92000003`) average. Keyed by council-area code (`S12…`) → joins postcodes.io `codes.admin_district`. **Why:** police.uk has no Police Scotland street-level data, so this is the area-level alternative. |
| `etl:scotland-council-tax` | `scotland-council-tax-by-laua.json` | **Scottish Government** "Council tax datasets" — two small **XLSX** from `gov.scot/publications/council-tax-datasets` (£-per-band "Council Tax by band" + "Chargeable dwellings" band counts), via SheetJS. Files are by LA *name* → mapped to `S12…` codes via an embedded 32-council lookup. Council-area (not data-zone) band mix; the £-per-band is the comparable figure. Reuses `CouncilTaxSummary` (+ `scope:"council"` so the panel labels it honestly). Joins `codes.admin_district`. |
...
| `etl:greenspace` | `greenspace.json` | **ONS** "Access to gardens and public green space in GB" (April 2020, OS data, OGL) — two **XLSX** via SheetJS. Public-green-space sheet "LSOA Parks and Playing Fields" → nearest-park distance (m) + count within 1 km, keyed by **2011 LSOA / data zone** (E01/W01/S01 → `codes.lsoa11`, an exact 2011 join). Gardens sheet "MSOA gardens" (2-row header) → % of addresses with private outdoor space, keyed by **MSOA** (`codes.msoa`; ~2011 so a few changed MSOAs miss — Scotland's IZs miss more, garden % then null). GB only (no NI). |
...
src/
  ...
  lib/   (one concern each)
    ...
    schools.ts      fetchSchools() / fetchSchoolsByIds() (GIAS+nurseries, URN-enriched; + Welsh/NI/Scottish register pins by nation; runtime fs reads), searchSchools()
    nations.ts      NATION_SCHOOL_INFO — per-nation {short, inspectorate, linkLabel} for non-England schools (SchoolCard/SchoolDetail/mapMarkers read it; the per-school link is on School.ofstedReport)
    reportCard.ts   new-framework EY report-card model + gradeDisplay()/gradeRank() (prefer report card over legacy grade)
    imd.ts  imdDomainsForLsoa() (England IMD 2019)   ·  wimd.ts  wimdForLsoa() (Wales WIMD 2025, 8 domains)   ·  simd.ts  simdForDatazone() (Scotland SIMD 2020v2, 7 domains; joins codes.lsoa11)   ·  nimdm.ts  nimdmForSoa() (NI NIMDM 2017, 7 domains; joins codes.lsoa11 = SOA 95…)   ·  broadband.ts  broadbandForLaua()   ·  mobile.ts  mobileForLaua() (Ofcom 4G/5G by LAUA)   ·  income.ts  incomeForMsoa() (ONS net household income by MSOA)  ·  greenspace.ts  greenSpaceForArea() (ONS nearest-park + garden share; lsoa11 + msoa, GB)  ·  affordability.ts  affordabilityForLaua() (ONS house-price-to-earnings ratio by LA)
    councilTax.ts councilTaxForLsoa()  (VOA band mix for the LSOA; runtime fs read like imd.ts)   ·  scotlandCrime.ts  scotlandCrimeForLaua() (council-area recorded crime; joins codes.admin_district)
    crime.ts        fetchCrime()  ·  prices.ts  fetchPrices()/fetchAddressSales()  ·  flood.ts  fetchFlood() (EA flood areas, England)  ·  devolvedFlood.ts  fetchDevolvedFlood(nation) — flood-RISK point-query for the devolved nations (Scotland SEPA ArcGIS identify; Wales NRW WMS GetFeatureInfo; NI DfI pending); PropertyChecks calls /api/devolved-flood for Scotland/Wales  ·  planning.ts  fetchPlanning() (nearby planning applications, PlanIt — runtime live fetch)  ·  planningConstraints.ts  fetchPlanningConstraints() (designations + listed buildings at a point, planning.data.gov.uk — runtime)  ·  census.ts  fetchCensus() (Census 2021 demographics by lsoa21, ONS/Nomis — runtime fetch, cached 30d)  ·  transport.ts  nearestStations() (nearest rail/metro/tram from committed stations.json) + stationsData()  ·  amenities.ts  nearbyAmenities() (counts from committed amenities.json + stations.json)  ·  evCharging.ts  nearbyEvCharging() (EV chargers near a point, committed OSM ev-charging.json)  ·  bathingWater.ts  nearestBathingWater() (nearest designated bathing water within ~10mi, committed EA bathing-waters.json)  ·  cqc.ts  nearbyCqc() (nearest rated GP/dentist/care home/hospital/home-care from committed cqc-locations.json + rating mix)  ·  airQuality.ts  airQualityForPoint() (background NO₂/PM2.5 from committed air-quality-by-grid.json, by OSGB easting/northing)
  ...
  components/
    ...
    DeprivationPanel · DemographicsPanel · CrimePanel · ScotlandCrimePanel · PricePanel · AmenitiesPanel · TransportPanel · EvChargingPanel · BroadbandPanel · MobilePanel · NoisePanel · AirQualityPanel · BathingWaterPanel · RankingsPanel  (area panels)
...
- **Area panels:** **Area rankings** (national-percentile summary), **Who lives here** (Census 2021
  ...
  E&W median); England &
  Wales), **Deprivation** (England **IMD 2019** 7-domain; Wales **WIMD 2025** 8-domain; Scotland **SIMD 2020v2** 7-domain; NI **NIMDM 2017** 7-domain — all four nations), Crime (vs national percentile), **Amenities** (committed OSM dataset — supermarkets,
...
- **Northern Ireland deprivation = NIMDM, also joined on `codes.lsoa11`.** NIMDM 2017
  (`nimdm.ts` / `nimdm-by-soa.json`) is on the **890 SOAs** (the 2001/2011 vintage, codes like
  `95GG20S1`). postcodes.io returns that SOA in `codes.lsoa11`, while `codes.lsoa` (= `lsoa21`) is the
  **2021** NI data zone (`N21…`) that NIMDM isn't on — so join `codes.lsoa11` (same field as Scotland,
  different code shape). 890/10 = 89 exactly → deciles are uniform (`ceil(rank/89)`), no band-fitting.
  CSV from Open Data NI (OGL), so no SheetJS. Like Scotland, postcodes.io's `index_of_multiple_deprivation`
  for NI is the live NIMDM rank (matches), but we use the committed file.
...
- **Catchment areas** (the big remaining gap) need **restricted NPD pupil-residence microdata** via
  the ONS Secure Research Service — *not* free/open. A distance-only approximation was built and then
  **removed** (a nearest-school-by-distance estimate needs so many caveats it confused more than it
  helped). Don't promise a clean build, and don't re-ship the distance proxy without a product rethink.
...
### Monitoring & scheduled refresh (the maintenance layer)

The app aggregates ~35 committed datasets + ~12 live upstreams, so the two failure modes that bite are
**silent staleness** (committed data quietly ages) and **silent breakage** (a runtime upstream goes dark
and the app fail-softs to "unavailable" — nobody notices until a user does). Two pieces make both visible:

- **`GET /api/health`** — reports every dataset's vintage and pings each runtime upstream (postcodes.io,
  police.uk, Land Registry, planning.data.gov.uk, PlanIt, EA flood, EPC, Nomis, SEPA + NRW flood, VOA).
  Returns `status: ok | degraded | down`. **`?strict=1`** returns **503** (not 200) when not `ok`, so an
  uptime pinger alerts on the non-2xx; **`?data=only`** skips the upstream pings (cheap freshness check).
  Ping every ~15–30 min, not every minute (each hit makes ~12 outbound requests). Upstream state is `up`
  (2xx/3xx), `reachable` (4xx — service answered; auth/params, the app's real request is fine) or `down`
  (5xx / timeout / network). It checks *reachability*, not schema correctness — that's the smoke test's job.
- **Data freshness** comes from **`src/data/data-manifest.json`** — record count, byte size and the **git
  commit date** ("vintage") of each dataset, generated by **`scripts/build-data-manifest.mjs`**
  (`npm run data:manifest`). It reads `git log`, so run it where full history exists (locally, or CI with
  `fetch-depth: 0`), **not** in the Vercel build (shallow clone → no history). The health route reads the
  committed manifest at runtime; a coarse per-cadence budget (`staleBudgetDays`) flags what's worth a look.
- **Scheduled refresh** — **`scripts/refresh.mjs <group>`** (`npm run refresh <group>`) re-runs a cadence
  group of ETLs and regenerates the manifest, printing a Markdown summary. Groups: `monthly` (registers +
  Ofsted), `school-results`, `area-annual`, `nations`, `deprivation`, `geo`, `all`.
  **`.github/workflows/refresh-data.yml`** runs the `monthly` group on a cron (2nd of each month) and any
  group on demand, then **opens a PR** with the regenerated data for review — it never writes to `main`.
  Uses the built-in `gh` CLI (no third-party actions). Pushing the workflow file needs a token with the
  **`workflow`** scope.

> **ETL-safety caveat:** some ETLs (e.g. `report-cards`, which scrapes live Ofsted pages) can overwrite
> their output with a near-empty file if the source fetch returns nothing (rate-limit, IP block, moved
> URL). The refresh workflow's **PR review + the manifest record-count diff** are the safety net — a drop
> like report-cards 1,098 → ~0 shows up plainly. **Never merge a refresh PR without a glance at the record
> counts.** Hardening the ETLs to abort (not overwrite) on an empty fetch is a worthwhile follow-up.
...
liveability sweep then also added **green space & garden access** (ONS/OS, GB — `etl:greenspace`),
after which free open-data *content* is
...
- **Restricted — free of charge, but accredited-access only.** **Real catchment** (where a school's
  ...
  Locrating's flagship — and we ship **no catchment feature at all**. A distance-based *approximation*
  (nearest state school of each phase by straight-line distance) was built and then **removed**: it needs
  so many caveats — real intake turns on siblings/faith/aptitude and the true last-distance-offered is
  usually *much smaller* — that it misled more than it informed.
...
- **Wales** — **effectively complete** for free data. Deprivation (WIMD 2025) and the **schools
  ...
  *named* list, so no rating-badge signal is achievable; the per-school My Local School link already
  carries the live Estyn report. School *quality* parity with England isn't possible. **Flood risk** is
  now shipped too (NRW hazard maps, live — `devolvedFlood.ts`).
- **Scotland** — **deprivation (SIMD 2020v2) + the schools register now shipped** (see §3/5/6/9). Scottish
  schools (not in GIAS) come from the SG "School Roll and Locations" register (`etl:scotland-schools`),
