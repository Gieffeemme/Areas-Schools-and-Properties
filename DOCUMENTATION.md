...
- **Northern Ireland** — **deprivation (NIMDM 2017) now shipped** (see §5/6/9; joins `codes.lsoa11`).
  Remaining: schools are *not in GIAS*; would need the **DE/EA** school register + **ETI** inspections
  (no single grade) + DE results. (police.uk *does* cover NI, so crime already works there.)
