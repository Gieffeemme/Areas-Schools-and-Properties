...
src/
  app/
    ...
    api/
      area/route.ts            geocode + schools + crime + prices + amenities + ev-charging + transport + broadband + mobile + noise + air-quality + bathing-water + census → AreaReport (cached 6h)
...
- **Area panels:** **Area rankings** (national-percentile summary), **Who lives here** (Census 2021
  ...
  hidden inland), Property prices, Property checks (EA flood + tenure + EPC energy ratings + **council-tax
...
added **EV charging**, a **Census expansion** (car/van availability + self-reported health) and **EA
bathing-water** quality. Further free, geography-joinable candidates still not built:
**bus stops** (NaPTAN — but low-signal/heavy), **DfT journey-time/connectivity** stats (mins to
GP/school/shops by LSOA; ~2019 vintage), **OHID Fingertips** local-health indicators, and **ONS
small-area income**. (**Census 2021 demographics** (+ car/van & health), **CQC health/care ratings**,
**air quality**, **mobile coverage**, **planning constraints**, **EV charging** and **bathing water**
shipped this round. Bathing water is the nearest EA designated water within ~10 mi (coastal, hidden
inland; see §9).
