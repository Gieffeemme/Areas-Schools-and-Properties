...
- **Wales** — deprivation (WIMD 2025) and the **schools register** now **shipped** (see §5/6/9). Welsh
  schools appear on the map/list/search with phase, pupils and Welsh-medium, from the Welsh-Gov address
  list (`etl:welsh-schools`), linking to My Local School. Remaining: **Estyn** grades for the badge
  model (narrative reports, no single judgement — needs a different model or a scrape) and Welsh
  **results** (sparse; no league tables). School *quality* parity with England isn't achievable.
