...
export default function AreasCompare({ initialPostcodes }: { initialPostcodes: string[] }) {
  ...
  const run = useCallback(async (pcs: string[]) => {
    ...
    setRows(settled);
    setLoading(false);
    const qs = new URLSearchParams({ mode: "areas", postcodes: list.join(",") });
  ...
  return (
    <>
      ...
      {loading && <p className="mt-6 text-sm text-[var(--muted)]">Fetching live data for each area…</p>}
